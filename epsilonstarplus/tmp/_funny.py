...
from util.map_util import obstruct_cell_list
...
class Robot:
    ...
    def retreat(self):
        ...
        return_path = self.reduce_path(return_path)
    ...
    def reduce_path(self, path):
        ...
        for idx, pos in enumerate(path):
            obstruct_cells = obstruct_cell_list(reduced_path[-1], pos)
            if any(self.map[x] == 'o' for x in obstruct_cells):
                reduced_path.append(path[idx - 1])
            if idx == len(path) - 1:
                reduced_path.append(pos)
...
def main():
    ...
    #     col_list = [file_name[13:-4], coverage_length, retreat_length, advance_length, total_travel_length, overlap_rate, return_charge_count, f'{extreme_deadlock_count}/{deadlock_count}', execute_time]
