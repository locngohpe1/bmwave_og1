...
class Robot:
    def __init__(self, battery_pos, map_row_count, map_col_count):
        ...
        map:
            ...
            'e': explored
            'o': obstacle (known)
            'n': new_obstacle (unknown, detected by sensor)
        '''
        self.map = None
        ...
        self.dynamic_obstacles = set()  # Track sensor-detected obstacles only
    ...
    def set_map(self, environment):
        ...
        self.known_environment = environment.copy()  # Store original known environment only
    ...
    def detect_new_obstacles(self):
        """Pure sensor detection - robot has NO prior knowledge of unknown obstacles"""
        current_pos = self.current_pos
        new_obstacles = []

        # 8-connected neighbors sensor detection (from Deep Q-learning paper)
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if dx == 0 and dy == 0:
                    continue
                ...
                # SENSOR DETECTION: Check UI's unknown_obstacles (robot has no prior knowledge)
                if (hasattr(ui, 'unknown_obstacles') and (x, y) in ui.unknown_obstacles and
                        (x, y) not in self.dynamic_obstacles):  # First time detection

                    self.map[x, y] = 'n'  # Mark as sensor-detected obstacle
                    self.dynamic_obstacles.add((x, y))
                    self.logic.weight_map[x, y] = -1  # Update BWave weight map
                    new_obstacles.append((x, y))
                    print(f"Sensor detected unknown obstacle at {(x, y)}")

        return new_obstacles

    def avoid_new_obstacle(self, blocked_cell, available_wp):
        """BWave-based obstacle avoidance using available waypoints"""
        current_pos = self.current_pos

        # Priority 1: Use BWave's alternative waypoints
        safe_wp = [wp for wp in available_wp if wp not in self.dynamic_obstacles]
        if safe_wp:
            return min(safe_wp, key=self.travel_cost)

        # Priority 2: 4-directional neighbors (BWave compatible)
        neighbors = [
    ...
    def run(self):
        ...
        while run:
            ...
            # SENSOR DETECTION IN ALL PHASES (coverage, retreat, advance)
            new_obstacles = self.detect_new_obstacles()
            if new_obstacles:
                print(f"Sensor detected obstacles in phase {self.move_status}: {new_obstacles}")
                time.sleep(0.3)
                ui.draw()
            ...
            # Pure BWave decision making with obstacle avoidance
            ...
            # Simple check: if BWave's choice hits sensor-detected obstacle, avoid
...
def main():
    ...
    if len(robot.dynamic_obstacles) > 0:
        print(f'Dynamic obstacles detected: {len(robot.dynamic_obstacles)}')
    else:
        print('Pure BWave performance - no dynamic obstacles')

    # BWave coverage metrics from paper
    total_free_cells = np.sum(robot.known_environment == 0)
    covered_cells = np.sum(robot.map == 'e')
    coverage_efficiency = (covered_cells / total_free_cells) * 100
    print(f'BWave coverage efficiency: {coverage_efficiency:.2f}%')
